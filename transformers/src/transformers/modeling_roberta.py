...
#     """
#     def __init__(self, smooth=1e-7, p=1, reduction='mean'):
...
#
#
# class BinaryDiceF05Loss(nn.Module):
#     """Dice loss of binary class
#     Args:
#         smooth: A float number to smooth loss, and avoid NaN error, default: 1
#         p: Denominator value: \sum{x^p} + \sum{y^p}, default: 2
...
#     """
#     def __init__(self, smooth=1e-7, p=1, reduction='mean'):
#         super(BinaryDiceF05Loss, self).__init__()
#         self.smooth = smooth
#         self.p = p
#         self.reduction = reduction
#
...
#         target = target.contiguous().view(target.shape[0], -1)
#
#         num = torch.sum(torch.mul(predict, target), dim=0)# + self.smooth
#         den = 0.95*torch.sum(predict.pow(self.p), dim=0) + 0.05*torch.sum(target.pow(self.p), dim=0) + self.smooth
#
#         loss = 1. - num / den
#
#         if self.reduction == 'mean':
