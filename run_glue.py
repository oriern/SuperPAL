...
def evaluate(args, model, tokenizer, prefix="", finalEval=False):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if args.output_mode == "classification":
            preds_prob = softmax(preds,axis=1)[:,1]#np.max(softmax(preds,axis=1), axis=1)
...
def main():
    ...
    parser.add_argument(
        "--calc_final_alignments", action="store_true", help="Set this flag if you want to calculate final aligments.",
    )
    ...
    # for num_labels(mnli)
    if args.model_name_or_path == 'roberta-large-mnli':
        num_labels_old = config_class.from_pretrained(args.model_name_or_path)._num_labels
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=prefix, finalEval = True)
